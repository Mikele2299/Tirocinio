...
def creazionefile():
 with open('analisi/profiles2.csv', 'w', newline='') as file:
    ...
    field = ["nome-cognome", "telefono","Email"]
...
def estrazione2(file):
 ...
 while(i<x):
  ...
  if('Phone' in stringa[i]  or 'ph' in stringa[i] or 'Ph' in stringa[i] or "Contact" in stringa[i] or stringa[i]=="MOBNO" or "Mob" in stringa[i]):
